"""Contrarian — devil's advocate council member.

Uses Claude. Challenges the Strategist's thesis by looking for crowded trades,
narrative-price divergences, and underweighted risks.
"""
...
logger = get_logger("agent.contrarian")

PROMPT_PATH = Path(__file__).resolve().parent.parent.parent / "prompts" / "contrarian.txt"
...
class Contrarian(BaseAgent):
    """Contrarian investment analyst who challenges the Strategist."""

    def __init__(self):
        super().__init__("contrarian", "council.contrarian")
    ...
    def generate_vote(
        ...
        """Generate the Contrarian's counter-thesis.
        ...
        Returns:
            CouncilVote with contrarian views.
        """
        signal_map = self._organize_signals(signals)

        feedback_section = ""
        if store:
            feedback = store.get_recent_feedback("contrarian", last_n=5)
            if feedback:
        ...
        return CouncilVote(
            ...
            views=views,
            summary=parsed.get("contrarian_thesis", ""),
        )
